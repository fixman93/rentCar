    ...
    return (
      <section className='last-added-item'>
        ...
        <div className='vihicle-price'>
          {project && project.Currency ? <div className='currency'>{project.Currency}</div> : null}
          {project && project.carPrice ? <div className='price'>{project.carPrice}</div> : null}
          <span>per day</span>
        </div>
      </section>
    )
